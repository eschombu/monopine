...
class Encoder(SequentialModel, EncoderBase):
    def __init__(self, conv_spec: Union[Conv1dSpec, Conv1dBlockSpec, Conv1dChainSpec], in_channels=1,
                 linear_spec: Optional[Union[LinearSpec, LinearBlockSpec, LinearChainSpec]] = None,
...
class TaskPredictor(SequentialModel):
    """ Produces predictions on pretext task using output of encoders, plus optional task-related input variables
    (e.g., segment separation distance)

    """
    def __init__(self, in_features: int, model_spec: Union[LinearSpec, LinearBlockSpec, LinearChainSpec]):
...
class PretextModel(nn.Module):
    def __init__(self, encoders: Union[nn.Module, Sequence[nn.Module]], task_predictor: nn.Module, encoded_segments=1,
                 loss=nn.BCELoss, optimizer=torch.optim.Adam, learning_rate=1e-3):
        super().__init__()
        if isinstance(encoders, nn.Module) and encoded_segments <= 1:
            self.encoder = encoders
        elif isinstance(encoders, nn.Module):
            self.encoder = MultiSegmentEncoder(*([encoders] * encoded_segments))
        else:
            self.encoder = MultiSegmentEncoder(*encoders)
    ...
    def fit(self, dataloaders: Union[DataLoader, Sequence[DataLoader]], epochs=3, rounds=3, loss=None, optimizer=None,
            ...
            for r in range(rounds):
                ...
                for i, dataloader in enumerate(dataloaders):
                    if len(dataloaders) > 1:
                        print(f"Training subset {i + 1} of {len(dataloaders)}")
